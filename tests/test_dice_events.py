...
def test_dice_service_emits_requested_and_fulfilled_for_expr():
    ...
    svc = DiceService(FakeRNG([3,3,4]), bus, cbus, ctx_provider)  # 3d6+2 => 12
    res = svc.roll_expression(str(uuid4()), "3d6+2")
...
def test_command_bus_routes_dice_commands():
    ...
    cbus = CommandBus()
    svc = DiceService(FakeRNG([6]), bus, cbus, ctx_provider)  # d20 adv/dis will use provided seq minimally
    cbus.register("dice.roll_expression", svc.handle)
    cbus.dispatch(Command(name="dice.roll_expression", payload={"expr":"1d6"}))
    bus.stop()
    assert seen["ok"] == 1
...
def test_meta_is_flattened():
    ...
    svc = DiceService(FixedRNG(), bus, cbus, ctx_provider)
    cbus.register("dice.roll_expression", svc.handle)
    cbus.dispatch(Command("dice.roll_expression", payload={
        "request_id": str(uuid4()),
        "expr": expr_adv(20),
        "meta":{"actor_id": "A1", "layer": "physical", "source": "tui", "debug": True}
    }))
